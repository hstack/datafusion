mod deep_project_physical;

use crate::optimizer_deep_indices::deep_project_physical::PushAllProjectionHints;
...
use datafusion_datasource_parquet::leaves::projection_specifier;
...
pub async fn fetch_parquet_metadata_test(
    store: &dyn ObjectStore,
    object_meta: &ObjectMeta,
    size_hint: Option<usize>,
    #[allow(unused)] decryption_properties: Option<Arc<FileDecryptionProperties>>,
...
async fn run_deep_projection_optimize_test(
    ...
    info!("Logical plan: {}", plan.display_indent());
    let optimized_plan = ctx.state().optimize(&plan).expect("Error optimizing plan");
    info!("Optimized plan: {}", optimized_plan.display_indent());
    let state = ctx.state();
    ...
    info!(
        "Physical plan: {}",
        displayable(physical_plan.deref()).indent(true)
    );
    let mut deep_projections_computed_with_physical_rule: Vec<DeepColumnIndexMap> =
        vec![];
    let _ = physical_plan.apply(|pp| {
        if let Some(dse) = pp.as_any().downcast_ref::<DataSourceExec>() {
            ...
            {
                deep_projections_computed_with_physical_rule.push(projection_specifier(
                    parquet_source.table_schema().file_schema().clone(),
                    // SAFETY - the projection function just makes an option, but the field exists
                    parquet_source.projection().unwrap(),
                    Some(&parquet_source.projection_hints),
                    &parquet_source.projection_hints_indices,
                ));
            }
        }
    ...
    info!(
        "Checking if plan has these deep projections: {:?} xxxxxxxxxxx {:?}",
        &deep_projections_computed_with_physical_rule, tests
    );
    assert_eq!(
        deep_projections_computed_with_physical_rule.len(),
        tests.len()
    );

    for i in 0..deep_projections_computed_with_physical_rule.len() {
        let l = deep_projections_computed_with_physical_rule[i].clone();
        let r = tests[i].clone();

        assert_eq!(l.len(), r.len(), "key count differs");
        for (k, lv) in l {
            let rv = r.get(&k).unwrap_or_else(|| panic!("missing key {k}"));
            let mut l = lv.clone();
            let mut r = rv.clone();
            l.sort();
            r.sort();
            assert_eq!(l, r, "values differ for key {k}");
        }
    }
    info!(
        "COMPUTED: {:?}",
        deep_projections_computed_with_physical_rule
    );
    info!("EXPECTED: {:?}", tests);
    let results = ctx
...
async fn test_deep_projections_1() -> Result<()> {
    ...
    let _ = run_deep_projection_optimize_test(
        ...
        "#,
        vec![
            HashMap::from([(0, vec![]), (1, vec![]), (2, vec!["aaid_id".to_string()]), (3, vec!["eVar56".to_string()])]),
            HashMap::from([(2, vec!["aaid_id".to_string()]), (3, vec!["eVar56".to_string()])])
        ],
    ).await?;
...
async fn test_deep_projections_genstudio() -> Result<()> {
    ...
    let _ = run_deep_projection_optimize_test(
        ...
        r#"
            ...
            LIMIT 100
        "#,
        vec![HashMap::from([(0, vec![]), (1, vec![]), (2, vec![]), (3, vec![]), (4, vec![])])],
    )
    .await?;
...
FROM
    ...
    "#;
    let _ = run_deep_projection_optimize_test(&ctx, query, vec![]).await?;
...
async fn test_mid_values_window() -> Result<()> {
    ...
    let _ = ctx
        .register_parquet(
            "extra_user_data",
            format!(
                "{}/tests/data/deep_projections/triplea/extra_user_data.parquet",
                env!("CARGO_MANIFEST_DIR")
            ),
            ParquetReadOptions::default(),
        )
    ...
    let _ = run_deep_projection_optimize_test(
        ...
        r#"
            SELECT
                extra_user_data.endUserIDs['_experience']['mcid']['name'],
                midvalues.timestamp,
                midvalues.web['webPageDetails']['pageViews']['value'] AS pageview,
                midvalues.endUserIDs['_experience']['mcid']['id'] AS mcid,
                midvalues.endUserIDs['_experience']['aaid']['id'] AS aaid,
                COALESCE(
                    midvalues.endUserIDs['_experience']['mcid']['id'],
                    midvalues.endUserIDs['_experience']['aaid']['id']
                ) AS partitionCol,
                LAG(timestamp) OVER(
                    PARTITION BY COALESCE(
                        midvalues.endUserIDs['_experience']['mcid']['id'],
                    ...
                    ORDER BY midvalues.timestamp
                ) AS last_event
            FROM
                midvalues
            INNER JOIN
                extra_user_data
            ON midvalues.endUserIDs['_experience']['mcid']['id'] = extra_user_data.endUserIDs['_experience']['mcid']['id']
            WHERE
                midvalues.timestamp >= TO_TIMESTAMP('2025-01-15')
                AND midvalues.timestamp < TO_TIMESTAMP('2025-01-16')
        ...
        vec![
            ...
            HashMap::from([
                (
                    ...
                    vec![
                        "_experience.mcid.id".to_string(),
                        "_experience.mcid.name".to_string(),
                    ],
                ),
            ]),
        ],
        ...
        vec![
            HashMap::from([
                ...
                (
                    1,
                    vec![
                        "webPageDetails.pageViews.value".to_string()
                    ]
                ),
                (
                    2,
...
async fn dump_parquet_schema(filename: &str) -> Result<SchemaDescriptor> {
    let meta = local_unpartitioned_file(filename);
    let store = Arc::new(LocalFileSystem::new());
    let metadata =
        fetch_parquet_metadata_test(store.deref(), &meta, None, None, None).await?;
...
async fn test_plain_map() -> Result<()> {
    ...
    let _ = dump_parquet_schema(filename.as_str()).await?;
...
async fn test_list_struct_map() -> Result<()> {
    ...
    let ctx = build_context();
    ctx.register_parquet("table1", filename.clone(), ParquetReadOptions::default())
        .await?;

    let query = r#"
    SELECT
        list_of_struct_of_list_of_struct[1]['fieldmap']['key1']['prop1']
    from
        table1"#;
    ...
    let _ = run_deep_projection_optimize_test(
        ...
        vec![
            HashMap::from(
                [(2, vec!["*.fieldmap.*.*.prop1".to_string()])]
            )
        ]
    ).await?;
    ...
    let query_planner = ctx.state().query_planner().clone();
    let physical_plan = query_planner
        .create_physical_plan(&optimized_plan, &ctx.state())
        .await
        .expect("Error creating physical plan");
    info!(
        "physical: {}",
    ...
    let results = collect(physical_plan, ctx.state().task_ctx()).await?;
    println!(
        "{}",
        pretty::pretty_format_batches_with_options(&results, &FormatOptions::default())?
            .to_string()
    );
...
#[tokio::test]
async fn test_list_struct_map_with_array_element() -> Result<()> {
    if !HANDLE_STRUCT_IN_LIST {
        info!("Test disabled !");
        return Ok(());
    }
    ...
    ctx.register_parquet("table1", filename.clone(), ParquetReadOptions::default())
        .await?;
    let query = r#"SELECT orderData.productList['SKU'] from table1"#;
    ctx.sql(query).await?.show().await?;

    let _ = run_deep_projection_optimize_test(
        &ctx,
        r#"SELECT orderData.productList.SKU from table1"#,
        vec![HashMap::from([(1, vec!["productList.*.SKU".to_string()])])],
    )
    .await?;
...
async fn test_xxx() -> Result<()> {
    ...
    let _ = run_deep_projection_optimize_test(
        ...
        vec![
            HashMap::from([
                ...
                (5, vec!["analytics.customDimensions.eVars.eVar40".to_string()]),
                (6, vec![])
            ])
        ]
    ).await?;
    ...
    info!("plan: {}", &plan);
    let optimized_plan = ctx.state().optimize(&plan)?;
    info!("optimized: {}", &optimized_plan.display_indent_schema());
    let query_planner = ctx.state().query_planner().clone();
    let physical_plan = query_planner
        .create_physical_plan(&optimized_plan, &ctx.state())
        .await
...
async fn test_identity_map() -> Result<()> {
    ...
    let ctx = build_context();
    ctx.register_parquet("raw", filename.clone(), ParquetReadOptions::default())
        .await?;
    ...
    let _ = run_deep_projection_optimize_test(
        ...
        r#"
        SELECT
            identityMap['ECID'][0]['id'] AS EntityID,
            'ECID' AS EntityType,
            timestamp AS RecordTime,
            unnest(
                make_array(
...
async fn test_gs_summary_metrics() -> Result<()> {
    ...
    let _ = run_deep_projection_optimize_test(
        ...
        vec![
            HashMap::from([
                (2, vec!["genStudioInsights.breakdownType".to_string(), "genStudioInsights.entityIDs.account.accountGUID".to_string()]),
                (3, vec![])
            ])
        ]
    ).await?;
...
#[tokio::test]
#[allow(dead_code, unused_variables)]
async fn test_delta_with_deep_struct_in_list() -> Result<()> {
    let filename = format!(
        "{}/tests/data/deep_projections/delta_with_deep_struct_in_list/values.parquet",
        env!("CARGO_MANIFEST_DIR")
    );

    let ctx = build_context();

    ctx.register_parquet("values", filename.clone(), ParquetReadOptions::default())
        .await?;

    let query = r#"
    SELECT
        productListItems['_experience']['analytics']['events']
    FROM
        values;
    "#;
    ...
    let _ = run_deep_projection_optimize_test(
        ...
        query,
        vec![HashMap::from([
            (2, vec!["*._experience.analytics.events".to_string()])
        ])],
    )
    .await?;
    ...
    let physical_plan = query_planner
        ...
        .await
        .expect("Error creating physical plan");
    info!("physical: {}", &optimized_plan.display_indent());
