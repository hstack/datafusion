...
use crate::optimizer_deep_indices::deep_project_physical::{projection_specifier, PushAllProjectionHints};
...
async fn run_deep_projection_optimize_test(
    ...
    let _ = physical_plan.apply(|pp| {
        if let Some(dse) = pp.as_any().downcast_ref::<DataSourceExec>() {
            ...
            {
                deep_projections.push(projection_specifier(pp));
            }
        }
    ...
    //     let l = deep_projections[i].clone().unwrap();
    //     let r = tests[i].clone().unwrap();
    //     assert_eq!(
    //         cleaned_up_projections(&l),
    //         cleaned_up_projections(&r),
    //         "Deep projections should be equal at index {}: got={:?} != expected={:?}",
    //         i, l, r
    //     )
    // }
    info!("COMPUTED: {:?}", deep_projections);
    info!("EXPECTED: {:?}", tests);
    Ok(())
}
...
async fn test_list_struct_map() -> Result<()> {
    ...
    let physical_plan = query_planner
        ...
        .await.expect("Error creating physical plan");
    info!("physical: {}", displayable(physical_plan.as_ref()).set_show_schema(true).indent(true));
    let results = collect(physical_plan, ctx.state().task_ctx()).await?;
    println!("{}", pretty::pretty_format_batches_with_options(&results, &FormatOptions::default())?
        .to_string());
...
async fn test_delta_with_deep_struct_in_list() -> Result<()> {
    ...
    FROM
        values;
    "#;
    // ctx.sql(query).await.unwrap().show().await.unwrap();
