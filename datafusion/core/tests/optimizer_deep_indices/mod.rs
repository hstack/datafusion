...
#[tokio::test]
async fn test_list_struct_map_projection_exprs() -> Result<()> {
    if !HANDLE_STRUCT_IN_LIST {
        info!("Test disabled !");
        return Ok(());
    }
    ...
    let arrow_schema = parquet_to_arrow_schema(parquet_schema, None).unwrap();
    let _df_schema = DFSchema::try_from(arrow_schema.clone()).unwrap();

    let deep_projection: HashMap<usize, Vec<String>> = HashMap::from([(
        2,
        vec!["*.fieldmap.*.*.prop1".to_string()],
    )]);

    let _pe = ProjectionExprs::from_projection_deep_indices(&[2], &deep_projection, &arrow_schema);
