...
impl PhysicalOptimizerRule for PushAllProjectionHints {
    fn optimize(
        ...
        let _ = plan.apply(|p| {
            if let Some(ds_exec) = p.downcast_ref::<DataSourceExec>() {
        ...
        let _ = plan.apply(|physical_plan| {
            if let Some(_ds_exec) =
                physical_plan.downcast_ref::<DataSourceExec>()
            {
...
pub fn get_expressions_to_check(
    ...
    if let Some(plan) = plan.downcast_ref::<ProjectionExec>() {
        return plan
            .expr()
            .iter()
            .map(|pe| pe.expr.clone())
    ...
    if let Some(plan) = plan.downcast_ref::<FilterExec>() {
        let mut out = vec![];
    ...
    if let Some(plan) = plan.downcast_ref::<SortExec>() {
        return plan
            .expr()
            .iter()
    ...
    if let Some(plan) = plan.downcast_ref::<AggregateExec>() {
    ...
    if let Some(plan) = plan.downcast_ref::<BoundedWindowAggExec>() {
        let mut out: Vec<Arc<dyn PhysicalExpr>> = vec![];
        plan.window_expr()
            .iter()
    ...
    if let Some(plan) = plan.downcast_ref::<WindowAggExec>() {
        let mut out: Vec<Arc<dyn PhysicalExpr>> = vec![];
        plan.window_expr()
            .iter()
            .for_each(|we| we.expressions().iter().for_each(|e| out.push(e.clone())));
    ...
    if let Some(plan) = plan.downcast_ref::<HashJoinExec>() {
        let mut out: Vec<Arc<dyn PhysicalExpr>> = vec![];
    ...
    // if let Some(plan) = plan.downcast_ref::<CrossJoinExec>() {}
    // if let Some(plan) = plan.downcast_ref::<NestedLoopJoinExec>() {}
    if let Some(plan) = plan.downcast_ref::<PiecewiseMergeJoinExec>() {
        return vec![plan.on.0.clone(), plan.on.1.clone()];
    }
    if let Some(plan) = plan.downcast_ref::<SortMergeJoinExec>() {
        let mut out: Vec<Arc<dyn PhysicalExpr>> = vec![];
        plan.on.iter().for_each(|oe| {
            out.push(oe.0.clone());
            out.push(oe.1.clone());
    ...
    if let Some(plan) = plan.downcast_ref::<SortPreservingMergeExec>() {
        let mut out: Vec<Arc<dyn PhysicalExpr>> = vec![];
        plan.expr().iter().for_each(|se| out.push(se.expr.clone()));
        return out;
    }
    if let Some(plan) = plan.downcast_ref::<SymmetricHashJoinExec>() {
        let mut out: Vec<Arc<dyn PhysicalExpr>> = vec![];
        plan.on().iter().for_each(|oe| {
            out.push(oe.0.clone());
            out.push(oe.1.clone());
...
pub fn replace_column_in_expr(
    ...
    let new_expr = expr
        .clone()
        .transform_down(|e| {
            if let Some(_col) = e.downcast_ref::<Column>()
                && !executed
            {
                executed = true;
...
pub fn find_sources_for_single_column_expr(
    ...
    if let Some(col) = get_first_column_from_expr(&expr) {
        ...
        if let Some(plan) = current.downcast_ref::<SortMergeJoinExec>() {
            let mut found = false;
        ...
        'outer: loop {
            ...
            if let Some(plan) = current.downcast_ref::<DataSourceExec>() {
            ...
            if let Some(filter_child) = current.downcast_ref::<FilterExec>() {
            ...
            if let Some(join) = current.downcast_ref::<HashJoinExec>() {
                if Arc::ptr_eq(current, plan) {
            ...
            if current.children().len() == 1
                && let Some(union) =
                current.children()[0].downcast_ref::<UnionExec>()
            {
                let mut out = vec![];
