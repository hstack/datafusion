...
pub fn simplified_parquet_column_paths(
        ...
        .flatten()
    {
        ...
        let col_index = schema.index_of(col_arg.name()).expect("Col in table");
        // let marker = format!("xx: {} {}", col_index, expr_to_deep_projection(&pexpr));
        let marker = simplified_parquet_column_path(&expr);
        paths.entry(col_index).or_default().push(marker);
    }
        ...
        .map(|(k, v)| {
            ...
            let newv = v
                ...
                .map(|v| {
                    ...
                    fix_simplified_column_path(v.as_str(), schema.field(k)).unwrap()
...
pub fn expr_is_get_field_or_array_or_cast_or_column(
...
pub fn extract_expressions_containing_column(
    input: &Arc<dyn PhysicalExpr>,
) -> Vec<Arc<dyn PhysicalExpr>> {
    let mut out = vec![];
    let _ = input.apply(|pe| {
        if expr_is_get_field_or_array_or_cast_or_column(pe) {
            out.push(pe.clone());
...
pub fn find_column_in_expr(expr: &Arc<dyn PhysicalExpr>) -> Option<Column> {
...
pub fn simplified_parquet_column_path(expr: &Arc<dyn PhysicalExpr>) -> String {
    // info!("expr_to_deep_projection: {:?}", expr);
