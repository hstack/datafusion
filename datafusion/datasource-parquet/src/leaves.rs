...
pub fn remap_top_level_field_indices(left: &SchemaRef, right: &SchemaRef) -> HashMap<usize, Option<usize>> {
    let mut out: HashMap<usize, Option<usize>> = HashMap::new();
    for (left_index, left_field) in left.fields.iter().enumerate() {
        if let Some((right_index, right_field)) =
            right.fields.find(left_field.name().as_str()) {
            out.insert(left_index, Some(right_index));
        } else {
            out.insert(left_index, None);
        }
    }
    out
}
...
pub fn parquet_leaf_paths(
    ...
    trace!(target: "deep", "parquet_leaf_paths {:?} {:?}", projection, projection_deep);
...
pub fn projection_specifier(
    ...
    for pexpr in projection_hints.into_iter().flatten() {
        ...
        trace!(target: "deep", "leaves::projection_specifier projection hint: {}", expr.to_string());
        if let Some(col_arg) = get_first_column_from_expr(&expr) {
            if let Ok(col_index) = source_schema.index_of(col_arg.name()) {
                let marker = simplified_parquet_column_path(&expr);
                trace!(target: "deep", "    > marker: {}", marker.as_str());
                let entry = deep_column_map.entry(col_index).or_default();
                if entry.contains("") {
                    // already full column !!!!!!!
                } else {
    ...
    trace!(target: "deep", "leaves::projection_specifier deep column map after handling projection hints: {:?}", &deep_column_map);
    ...
    let final_map: HashMap<usize, Vec<String>> = deep_column_map
        ...
        .map(|(k, v)| {
            ...
            let mut newv = v
                ...
                .map(|v| {
                    trace!(target: "deep", "fix_deep for {}", v.as_str());
                    fix_simplified_column_path(v.as_str(), source_schema.field(k))
                    // if let Ok(cp) = fix_simplified_column_path(v.as_str(), source_schema.field(k)) {
                    //     Ok(cp)
                    // } else {
                    //     "".to_string()
                    // }
        ...
        .into_iter()
        .collect();
    trace!(target: "deep", "leaves::projection_specifier final map: {:?}", &final_map);
    // info!(target: "deep", "leaves::projection_specifier <<<<<<<<<<<<<<<<<<<<<<<<<<");
    final_map
}
...
pub fn expr_is_only_get_field_or_array_or_cast_and_contains_column(
    ...
    let _ = input.apply(|pe| {
        if let Some(sfe) = pe.downcast_ref::<ScalarFunctionExpr>()
            && sfe.name() == "get_field"
        {
        ...
        if let Some(_) = pe.downcast_ref::<CastExpr>() {
            return Ok(TreeNodeRecursion::Continue);
        };
        if let Some(_) = pe.downcast_ref::<Column>() {
            has_column = true;
...
pub fn simplified_parquet_column_path(expr: &Arc<dyn PhysicalExpr>) -> String {
    ...
    let _ = expr.apply(|e| {
        if let Some(_) = e.downcast_ref::<Column>() {
        ...
        if let Some(sfe) = e.downcast_ref::<ScalarFunctionExpr>()
            && sfe.name() == "get_field"
        {
            if sfe.args().len() == 2 {
                if let Some(literal) = sfe.args()[1].downcast_ref::<Literal>() {
                    match literal.value() {
    ...
    'outer: loop {
        ...
        match current.data_type() {
            DataType::Map(map_struct, _) => {
                ...
                if let DataType::Struct(map_struct_fields) = map_struct.data_type() {
                    let (_idx, field) = map_struct_fields
                        .find("value")
                        .expect("map struct should have field with name 'value'");
                    current = field;
                    continue 'outer;
                }
            }
            ...
            DataType::Struct(inner) => {
                // FIXME @HStack - this used to crash, but we have a problem with
                // aep datasets and observable schemas, which are only at the logical level !!!!
                // Making this return whatever it can, and not crash
                if let Some((_idx, field)) = inner.find(pieces[idx]) {
                    out_pieces.push(pieces[idx].to_string());
                    current = field;
                    idx += 1;
                    continue 'outer;
                } else {
                    warn!("struct should have field with name {}", pieces[idx]);
                    return internal_err!("struct should have field with name {}", pieces[idx]);
                }
            }
            _ => {
                error!(
