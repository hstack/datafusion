...
use crate::{project_schema, DataFusionError};
use arrow::array::{
    as_fixed_size_list_array, as_generic_list_array, as_struct_array, new_null_array,
...
pub fn rewrite_record_batch_fields(
    ...
    for i in 0..dst_fields.len() {
        ...
        if src_field_opt.is_some() {
            ...
            let src_arr = Arc::<dyn Array>::clone(&arrays[src_idx]);
        ...
        } else if fill_missing_source_fields {
...
pub fn rewrite_record_batch_field(
    ...
    if arrow_cast_available {
        ...
        return Ok((casted_array, Arc::clone(&dst_field)));
    }
    match (src_field.data_type(), dst_field.data_type()) {
        (DataType::List(src_inner), DataType::List(dst_inner)) => {
            if data_type_recurs(src_field.data_type()) {
                let src_array_clone = Arc::<dyn Array>::clone(&src_array);
                ...
                let src_nulls = src_inner_list_array.nulls().cloned();
                let (values, field) = rewrite_record_batch_field(
                    Arc::clone(dst_inner),
                    Arc::clone(src_inner),
                    Arc::clone(src_inner_list_array.values()),
                    num_rows,
                    fill_missing_source_fields,
                    error_on_missing_source_fields,
                )?;
                let nlarr = ListArray::try_new(
                    Arc::<Field>::clone(&field),
                    src_offset_buffer,
                    values,
                    src_nulls,
                );
                let list_field = Arc::new(Field::new(
                    ...
                    DataType::List(Arc::<Field>::clone(&field)),
                    dst_field.is_nullable(),
                ));

                Ok((Arc::new(nlarr?), list_field))
            } else {
                let casted_array = cast(src_array.as_ref(), dst_field.data_type())?;
                Ok((casted_array, Arc::clone(&dst_field)))
            }
        }
        (
            ...
            if data_type_recurs(src_field.data_type()) {
                let tmp = Arc::clone(&src_array);
                let src_inner_list_array = as_fixed_size_list_array(tmp.as_ref()).clone();
                let src_nulls = src_inner_list_array.nulls().cloned();
                let (values, field) = rewrite_record_batch_field(
                    Arc::clone(dst_inner),
                    Arc::clone(src_inner),
                ...
                let list_field = Arc::new(Field::new(
                    dst_field.name().clone(),
                    DataType::FixedSizeList(Arc::clone(&field), *dst_sz),
                    dst_field.is_nullable(),
                ));
        ...
        (DataType::Map(src_inner, _), DataType::Map(dst_inner, dst_ordered)) => {
            match (src_inner.data_type(), dst_inner.data_type()) {
                (DataType::Struct(src_inner_f), DataType::Struct(dst_inner_f)) => {
                    ...
                    let src_nulls = src_map.nulls().cloned();
                    let src_offset_buffer = src_map.offsets().clone();

                    let (tmp_values_array, tmp_values_field) =
                        rewrite_record_batch_field(
                            Arc::clone(&dst_inner_f[1]),
                            Arc::clone(&src_inner_f[1]),
                            Arc::<dyn Array>::clone(src_map.values()),
                            num_rows,
                            fill_missing_source_fields,
                            error_on_missing_source_fields,
                        )?;
                    ...
                    let entry_struct = StructArray::from(vec![
                        (Arc::clone(&dst_inner_f[0]), Arc::clone(src_map.keys())),
                    ...
                    let map_field = Arc::new(Field::new(
                        dst_field.name().clone(),
                        DataType::Map(Arc::clone(&struct_field), *dst_ordered),
                        dst_field.is_nullable(),
                    ));
        ...
        (DataType::Struct(src_inner), DataType::Struct(dst_inner)) => {
            ...
            let src_nulls = src_struct_array.nulls().cloned();
            ...
            let struct_field =
                Field::new_struct(dst_field.name(), dst_fields, dst_field.is_nullable());
...
pub fn try_rewrite_record_batch_with_mappings(
    ...
    let field_vecs = dst
        ...
        .map(|(field, src_idx)| match src_idx {
            Some(batch_idx) => {
                let arr = Arc::<dyn Array>::clone(&src_record_batch_cols[batch_idx]);
            ...
            None => Ok((new_null_array(field.data_type(), num_rows), Arc::clone(field))),
        })
...
pub fn can_rewrite_schema(
    dst_schema: &SchemaRef,
    src_schema: &SchemaRef,
    fill_missing_source_fields: bool,
) -> bool {
    let dst_fields = &dst_schema.fields;
    let src_fields = &src_schema.fields;
    can_rewrite_fields(dst_fields, src_fields, fill_missing_source_fields)
}
...
pub fn has_deep_projection(possible: Option<&HashMap<usize, Vec<String>>>) -> bool {
    ...
    !(tmp.is_empty() || tmp.iter().all(|(_k, v)| v.is_empty()))
}
...
pub fn rewrite_schema(
            ...
            && path_prefix_exists(filters, &src_field_path)
        {
            let out = match src_field.data_type() {
                ...
                DataType::Struct(src_inner) => {
                    let dst_fields =
                        rewrite_schema_fields(src_field_path.clone(), parent_flags, src_inner, filters);
    ...
    let splatted = splat_columns(Arc::clone(&src), &actual_projection, projection_deep);
...
mod tests {
    ...
    fn test_schema_rewrite_with_similar_column_names() {
        ...
        let rewritten_schema = rewrite_schema(Arc::clone(&schema), &projection, &projection_deep);
