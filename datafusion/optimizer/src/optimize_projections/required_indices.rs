...
impl RequiredIndicies {
    ...
    fn add_expr(&mut self, input_schema: &DFSchemaRef, expr: &Expr, deep_indices: Option<&Vec<String>>) {
        ...
        for (col, rest) in cols {
            if let Some(idx) = input_schema.maybe_index_of_column(&col) {
                ...
                for tmp in &rest {
                    ...
                    // ADR: FIXME
                    let fixed = fix_possible_field_accesses(&input_schema.clone(), idx, &tmp_pieces).unwrap();
