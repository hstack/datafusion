...
fn rewrite_projection_given_requirements(
    ...
    let required_indices =
        indices.clone().with_exprs_and_old_indices(input.schema(), &expr)?;
    ...
    // projection down
    let opt1 = optimize_projections(Arc::unwrap_or_clone(input), config, required_indices)?;
    opt1.transform_data(|input| {
