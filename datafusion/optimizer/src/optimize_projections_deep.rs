...
impl Debug for PlanWithDeepColumnMap {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        ...
        f.write_str(format!("{} {:?}", plan_name, self.columns).as_str())
    }
}
...
pub fn deep_plan_transformer(
    ...
    plan.transform_down(|plan| {
        ...
        if options.optimizer.deep_column_pruning_flags & FLAG_ENABLE_SUBQUERY_TRANSLATION != 0 {
                    ...
                    match expr {
                        Expr::Alias(Alias { expr: alias_source, relation: _relation, name }) => {
                            ...
                            if alias_source_column_map.len() != 1 {
                                warn!("1 subquery translation: fix aliased cols from above - strange result for map: {:?}", &alias_source_column_map);
                            } else {
                        ...
                        _ => {
                            ...
                            if column_map.len() != 1 {
                                warn!("2 subquery translation: fix aliased cols from above - strange result for map: {:?}", &column_map);
                            } else {
            ...
            trace!(target: "deep", "subquery translation: found in current plan: {:?}", aliases_in_current_plan);
            trace!(target: "deep", "subquery translation: found in current plan: {:?}", &cols_from_above);
            ...
            for PlanWithDeepColumnMap { plan, columns } in cols_level.iter() {
                ...
                for (col, projections) in columns.iter() {
                    if col.relation.is_none() && aliases_in_current_plan.contains_key(&col.name) {
                        ...
                        // trace!(target: "deep", "subquery translation: replacing column {:?} with {:?} = {:?}", col, actual_column, projections);
            ...
            trace!(target: "deep", "subquery translation: REPLACED: {:?}", &cols_level);
        }

        let current_pdc = PlanWithDeepColumnMap::from_plan(&plan);
        info!("CURRENT PDC: {:?}", &current_pdc.columns);
        cols_level.push(current_pdc);
        // trace!(target: "deep", "BEFORE: {:?}", &cols_level);
        ...
        match plan {
            LogicalPlan::Projection(proj) => {
                ...
                if options.optimizer.deep_column_pruning_flags & FLAG_ENABLE_PROJECTION_MERGING != 0 {
                    // trace!(target: "deep", "projection merging: EXECUTING FLAG_ENABLE_PROJECTION_MERGING code path, cols_level = {:?}", &cols_level);
                    // trace!(target: "deep", "projection merging: EXECUTING FLAG_ENABLE_PROJECTION_MERGING expr = {:?}", &expr);
                    if are_exprs_plain(&expr) && cols_level.len() > 0 {
                        // trace!(target: "deep", "projection merging: can proceed, projection is plain");
                        ...
                        'col_iterate: for (my_col, my_deep_projections) in my_pdc.columns.iter() {
                            // trace!(target: "deep", "projection merging searching previous projections for column {:?}, current projections={:?}", my_col, my_deep_projections);
                            for my_deep_projection in my_deep_projections.iter() {
                                ...
                                if my_deep_projection == "*" {
                                    ...
                                    // trace!(target: "deep", "projection merging found previous deep projections for column {:?}: {:?}", my_col, &previous_deep_projections);
                                    ...
                                    // trace!(target: "deep", "projection merging rewrite projection for col {:?}: {:?}", my_col, previous_deep_projections);
                        ...
                        if my_new_modified_columns.len() > 0 {
                            ...
                            // {
                            // here, we need to add stars according to schema
                            let corrected_deep_projections = fix_deep_projection_according_to_table_schema_for_plain_schema(&cols_level.last().unwrap().columns, proj_inner_schema);
                            // }
                            ...
                            trace!(target: "deep", "REWRITE SCHEMA: {:?} {:?}", &all_projection_indices, &deep_projection);
                            ...
                            trace!(target: "deep", "REWRITTEN SCHEMA: {:#?}", new_proj_inner_schema);
            ...
            LogicalPlan::TableScan(table_scan) => {
                ...
                trace!(target: "deep", "TABLE SCAN: input cols_level= {:?}, projection={:?}, filters: {:?}", &cols_level, &projection, &filters);
                ...
                trace!(target: "deep", "TABLE SCAN COLUMNS FOR ME 1({}): {:?}", table_name.table(), columns_for_this_scan);
                ...
                trace!(target: "deep", "TABLE SCAN COLUMNS FOR ME 2({}): {:?}", table_name.table(), single_columns_for_this_scan);
                ...
                trace!(target: "deep", "TABLE SCAN COLUMNS FOR ME 3({}): {:?}", table_name.table(), fixed_single_columns_for_this_scan);
                ...
                trace!(target: "deep", "Rewriting deep projections for table {}: {:?} {:?}", table_name.table(), projection.clone(), projection_deep);
                ...
                trace!(target: "deep", "reprojected schema: {:#?}", reprojected_schema.inner());
            ...
            LogicalPlan::SubqueryAlias(sqa) => {
                ...
                if options.optimizer.deep_column_pruning_flags & FLAG_ENABLE_SUBQUERY_TRANSLATION != 0 {
                    // trace!(target: "deep", "subquery translation: activating subquery translation, for alias {}", &alias);
                    // trace!(target: "deep", "subquery translation: activating subquery translation CHECKING {:?}", &cols_level);
                    ...
                    trace!(target: "deep", "subquery translation: translated pdcs = {:?}", translated_pdcs);
                }
...
pub fn get_columns_referenced_in_plan(plan: &LogicalPlan) -> DeepColumnColumnMap {
    let expressions = plan.expressions(); //get_plan_expressions(p);
                                          // trace!(target: "deep", "  expressions = {:?}", &expressions);

    let mut is_table_scan = false;
    let mut table_reference: Option<TableReference> = None;
    // if let LogicalPlan::TableScan(table_scan) = plan {
    //     is_table_scan = true;
    //     table_reference = Some(table_scan.table_name.clone());
    // }
    ...
    for expr in expressions.iter() {
        ...
        for (k, vs) in tmp.iter() {
            ...
            if is_table_scan {
                ...
                kclone.relation = table_reference.clone();
                for v in vs {
                    merge_value_in_column_map(&mut deep_column_map, &kclone, v.clone());
                }
            } else {
            ...
            // trace!(target: "deep", "  column = {:?}, vs: {:?}", k, vs);
    ...
    // trace!(target: "deep", "columns : {:?}", &deep_column_map);
    deep_column_map
}
...
pub fn fix_deep_projection_according_to_table_schema_for_plain_schema(
    ...
    for (col, deep_projection_specifiers) in input.iter() {
        ...
        if let Ok(col_idx_in_schema) = schema.index_of(&name) {
            trace!(target: "deep", "COL IDX {}", col_idx_in_schema);
            ...
            for projection_specifier in deep_projection_specifiers {
                trace!(target: "deep", "checking projection specifier {}", projection_specifier);
                // ADR: FIXME
...
pub fn fix_possible_field_accesses(
    ...
    'outer: while index < deep_projection_specifier_pieces.len() {
        trace!(target: "deep", "  at index: {}, value: {}, field: {:?}", index, deep_projection_specifier_pieces[index], field);
        match field.clone().data_type() {
            // scalars
            DataType::Null
            | DataType::Boolean
            | DataType::Int8
            ...
            DataType::Struct(inner_struct) => {
                for f in inner_struct.iter() {
                    if f.name() == &deep_projection_specifier_pieces[index] {
                        ...
                        trace!(target: "deep", "FOUND FIELD, continuing iteration");
                        out.push(deep_projection_specifier_pieces[index].clone());
                        index += 1;
                        field = f.clone();
                        continue 'outer;
            ...
            DataType::Map(inner_map, _) => {
                match inner_map.data_type() {
                    DataType::Struct(inner_map_struct) => {
                        field = inner_map_struct[1].clone();
...
/// - this can handle if we add a projection that is already included (adding a.b.c when we already hava a)
