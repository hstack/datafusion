...
use datafusion_common::{internal_err, Column, DFSchema, DFSchemaRef, DataFusionError, Result, ScalarValue, TableReference};
...
impl OptimizerRule for OptimizeProjectionsDeep {
    ...
    fn rewrite(
        ...
        debug!(target: "deep", "optimize_projections_deep: plan={}", plan.display_indent_schema());
        let options = config.options();
        if options.optimizer.deep_column_pruning_flags & FLAG_ENABLE != 0 {
            let plan_clone = plan.clone();
            let new_plan_transformed = deep_plan_transformer(plan_clone, &[], &options)?;
            // @DeepProjections
            if new_plan_transformed.transformed {
                // we have deep projections
                // we extract all the deep projections, and copy them to the TableScans in the original plan
                // we throw away the modified plan
                let plan_with_deep_projections = new_plan_transformed.data;

                // find the table scans from the new plan
                let mut new_plan_table_scan_deep_projections = vec![];
                let _ = plan_with_deep_projections.apply(|lp| {
                    if let LogicalPlan::TableScan(table_scan) = lp {
                        new_plan_table_scan_deep_projections.push(table_scan.projection_deep.clone());
                    }
                    Ok(TreeNodeRecursion::Continue)
                });

                let mut ts_index = 0usize;
                let original_plan_with_deep_projections = plan.transform_down(|lp| {
                    if let LogicalPlan::TableScan(table_scan) = lp {
                        // @DeepProjection: the table_scan.source.schema() - might contain things that are NOT
                        // actually in the file schema, like extra cols for delta
                        // we'll clean up the projections in opener
                        let new_table_scan = TableScan {
                            projection_deep: new_plan_table_scan_deep_projections[ts_index].clone(),
                            ..table_scan
                        };
                        ts_index += 1;
                        // info!("AFTER TABLE SCAN: {:?}", new_table_scan);
                        // info!("AFTER TABLE SCAN deep projection: {:?}", new_table_scan.projection_deep);
                        // info!("AFTER TABLE SCAN source schema: {:?}", new_table_scan.source.schema());
                        Ok(Transformed::yes(LogicalPlan::TableScan(new_table_scan)))
                    } else {
                        Ok(Transformed::no(lp))
                    }
                });
                original_plan_with_deep_projections
            } else {
...
pub fn deep_plan_transformer(
    plan: LogicalPlan,
    cols_from_above: &[PlanWithDeepColumnMap],
    options: &ConfigOptions,
) -> Result<Transformed<LogicalPlan>> {
    let mut cols_level: Vec<PlanWithDeepColumnMap> = Vec::from(cols_from_above);
    trace!(
        target: "deep",
    ...
    plan.transform_down(|plan| {
        ...
        match plan {
            LogicalPlan::Projection(proj) => {
                ...
                if options.optimizer.deep_column_pruning_flags & FLAG_ENABLE_PROJECTION_MERGING != 0 {
                    ...
                    if has_previous_projection && !cols_level.is_empty() {
                        ...
                        let mut new_proj_fields: Vec<FieldRef> = vec![];
            ...
            LogicalPlan::Join(join) => {
                ...
                let new_left = deep_plan_transformer(left.as_ref().clone(), &cols_level, options)?;
                let new_right = deep_plan_transformer(right.as_ref().clone(), &cols_level, options)?;
            ...
            LogicalPlan::Union(union) => {
                ...
                for input in inputs.iter() {
                    let tmp = deep_plan_transformer(input.as_ref().clone(), &[], options)?;
            ...
            LogicalPlan::TableScan(table_scan) => {
                ...
                let columns_for_this_scan = filter_expressions_for_table(&cols_level_as_simple_map, table_name.table());
            ...
            LogicalPlan::Subquery(sq) => {
                ...
                let subquery_result = deep_plan_transformer(subquery.as_ref().clone(), &[], options)?;
...
pub fn reproject_for_deep_schema(
    ...
    if let (Some(projection), Some(projection_deep)) = (projection, projection_deep) {
        ...
        .apply(|expr| {
            match expr {
                Expr::Column(qc) => {
                    ...
                    if tmp.is_empty() {
                        // entire column
                        append_column::<Column>(&mut accum, qc, &["*".to_string()]);
                    } else {
                        ...
                        "get_field" => {
                            if sf.args.len() == 2 {
                                ...
                                sf.args
                                    ...
                                    .map(|(i, arg)| {
                                        match arg {
                                            Expr::Literal(ScalarValue::Utf8(Some(str)), _) => {
                                                Ok(str.clone())
                                            },
                                            Expr::Literal(_lit_expr, _) => {
                                                error!(
                                                        "Can't handle get_field argument at position {} {:?}",
                                                        i,
                                                        sf.args[1]
                                                    );
                                                in_other_literal_call = true;
                                            ...
                                            _ => {
                                                error!("Can't handle get_field argument at position {} {:?}", i, arg);
                                                internal_err!("field argument")
                                            }
                                        }
                                    })
...
pub fn append_column<T>(acc: &mut HashMap<T, Vec<String>>, column: &T, rest: &[String])
where
